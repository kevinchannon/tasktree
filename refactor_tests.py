...
def refactor_file(file_path: Path) -> None:
    ...
    content = re.sub(r'\bfrom tasktree\.parser import.*\bEnvironment\b',
    ...
    content = re.sub(r'^\s*environments:\s*$', lambda m: m.group(0).replace('environments:', 'runners:'), content, flags=re.MULTILINE)
    ...
    content = re.sub(r'# .*environment definition', lambda m: m.group(0).replace('environment definition', 'runner definition'), content)
    content = re.sub(r'# .*Environment definition', lambda m: m.group(0).replace('Environment definition', 'Runner definition'), content)
    content = re.sub(r'# .*Docker environment', lambda m: m.group(0).replace('Docker environment', 'Docker runner'), content)
