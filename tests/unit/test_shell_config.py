...
class TestShellLookup(unittest.TestCase):
    ...
    def test_bash_maps_to_bash(self):
        """Test that 'bash' maps to ['bash'] (script-file execution, no -c)."""
        self.assertEqual(SHELL_LOOKUP["bash"], ["bash"])

    def test_sh_maps_to_sh(self):
        """Test that 'sh' maps to ['sh'] (script-file execution, no -c)."""
        self.assertEqual(SHELL_LOOKUP["sh"], ["sh"])

    def test_zsh_maps_to_zsh(self):
        """Test that 'zsh' maps to ['zsh'] (script-file execution, no -c)."""
        self.assertEqual(SHELL_LOOKUP["zsh"], ["zsh"])

    def test_fish_maps_to_fish(self):
        """Test that 'fish' maps to ['fish'] (script-file execution, no -c)."""
        self.assertEqual(SHELL_LOOKUP["fish"], ["fish"])
    ...
    def test_powershell_maps_to_powershell_file(self):
        """Test that 'powershell' maps to the -File invocation form."""
        self.assertEqual(
            SHELL_LOOKUP["powershell"],
...
class TestShellConfig(unittest.TestCase):
    ...
    def test_shell_config_creation_with_cmd_and_preamble(self):
        ...
        config = ShellConfig(cmd=["bash"], preamble="set -euo pipefail")
        self.assertEqual(config.cmd, ["bash"])
    ...
    def test_shell_config_creation_with_cmd_only(self):
        ...
        config = ShellConfig(cmd=["sh"])
        self.assertEqual(config.cmd, ["sh"])
    ...
    def test_shell_config_preamble_defaults_to_empty_string(self):
        ...
        config = ShellConfig(cmd=["bash"])
        self.assertEqual(config.preamble, "")

    def test_shell_config_cmd_is_list(self):
        """Test that cmd is stored as a list."""
        config = ShellConfig(cmd=["powershell", "-ExecutionPolicy", "Bypass", "-File"])
...
class TestParseShellConfig(unittest.TestCase):
    ...
    def test_cmd_as_string_shorthand(self):
        ...
        self.assertEqual(config.cmd, ["zsh"])
    ...
    def test_cmd_as_list_verbatim(self):
        """Test that shell: {cmd: [/usr/local/bin/bash]} is passed through as-is."""
        config = parse_shell_config({"cmd": ["/usr/local/bin/bash"]}, "test-runner")
        self.assertEqual(config.cmd, ["/usr/local/bin/bash"])

    def test_cmd_as_list_with_preamble(self):
        """Test that preamble is parsed alongside cmd list."""
        config = parse_shell_config({"cmd": ["bash"], "preamble": "set -euo pipefail"}, "test-runner")
        self.assertEqual(config.cmd, ["bash"])
        self.assertEqual(config.preamble, "set -euo pipefail")
    ...
    def test_non_string_preamble_raises_value_error(self):
        ...
        with self.assertRaises(ValueError) as cm:
            parse_shell_config({"cmd": ["bash"], "preamble": 123}, "test-runner")
