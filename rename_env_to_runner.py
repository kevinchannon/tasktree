...
def main():
    ...
    python_replacements = [
        ...
        (
            r'"env"(?=\s*[:)])',
            r'"run_in"',
        ),  # Only replace "env" when followed by : or )
    ]
    ...
    json_files = [
        f for f in json_files if ".venv" not in str(f) and "node_modules" not in str(f)
    ]
    ...
    print(
        f"  Total files modified: {len(modified_py) + len(modified_yaml) + len(modified_md) + len(modified_json)}"
    )
